...
adb_ddl = ADBDDL()
agent_database = st.session_state['agent_database'] if 'agent_database' in st.session_state else 'default'
...
def validate_response(response, chain, chat_history, vectorstore, enabled=False):
    if enabled:
        ...
        validation_faiss_memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
        validation_query = f"{chains_templates()['validation_prompt']} \n {response}"
        return chain.run({"question": validation_query, "chat_history": chat_history}, vectorstore, validation_faiss_memory)
    else:
        return response
...
def run_chat_chain(chain, query, chat_history):
    with st.spinner(text="Executing LLM Chain..."):
        ...
        faiss_vectorstore = VectorStoreHandler(**{"vectorstore":"faiss"}).get_vectorstore()
        faiss_result = chain.run({"question": query, "chat_history": chat_history}, faiss_vectorstore, faiss_memory)
        update_progress_bar(44, 'submit', submit_progress_bar)
        result = validate_response(faiss_result, chain, chat_history, faiss_vectorstore, enabled=False)
...
def chat_loop(chain):
    ...
    if selection == 'chain':
        ...
        if len(query) > 2 and submit_button:
            result, messages = run_chat_chain(chain=chain, query=query, chat_history=chat_history)
...
def run_generated_code(result_query):
    ...
    user_query = st.text_area(label=f'Run Query: {code}:', value=code, height=200, label_visibility="hidden")
